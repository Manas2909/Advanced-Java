...
in the url of the mysql database passed as 1st argument to DriverManager.getConnection(), jdbc is main protocol, mysql is the name of the database, localhost is the ip address or host name of the pc on which the mysql database is running, and because it is running on same pc as the client jdbc java pgm so we have mentioned the string name as loalhost instead of the ipaddress or host name in the url argument passed to getConnection(). 3306 is the port number on which mysql database listens to incming clients. This port number can be anything and must be set when we install the mysql database on the system. database1 is the name of the sub database in mysql in which we want to create tables and store data in the tables in the mysql database. So first we need to also create the database1 sub database in mysql database by going to mysql command prompt, typing password, then typing create database subdatabasename;
